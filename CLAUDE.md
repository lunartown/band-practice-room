...
- **선형(일자) 히스토리를 유지한다.** 머지 커밋을 남기지 말고 rebase(또는 squash)로 한 줄 히스토리를 지킨다.
- 브랜치명은 의미 있게 짓는다. 무엇을 하는지 드러나는 이름(`fix-vercel-backend-load`, `feat-favorite-share` 등)을 쓰고, 임의 해시·번호만으로 짓지 않는다.
