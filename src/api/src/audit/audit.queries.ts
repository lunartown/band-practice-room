// db-audit 스킬(.claude/skills/db-audit/SKILL.md)의 읽기 전용 진단 쿼리를 서버에서 실행하기 위한 정의.
// 여기 있는 SQL은 전부 SELECT 전용이며, 실행 시 세션을 read-only 로 고정한다(audit.service.ts).
// 무제한 결과가 나올 수 있는 쿼리에는 LIMIT 을 걸어 응답 크기를 제한한다.
