...
export async function shareStudio(name: string, url: string | null): Promise<void> {
  // OS 공유 시트에서 실제로 보냈는지는 알 수 없다(취소도 예외로 떨어진다).
  // 그래서 공유 "의도"를 센다 — 이름도 share_click 이다.
  track('share_click', { target: 'studio', has_url: url != null });
...
export async function shareApp(): Promise<void> {
  track('share_click', { target: 'app', has_url: true });
